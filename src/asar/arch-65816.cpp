...
#define as_a(   op, byte) if (is(op)) { if(!explicitlen && !hexconstant) asar_throw_warning(0, warning_id_implicitly_sized_immediate); if (!check_immediate_bitwidth_a(len)) 	{ asar_throw_warning(0, warning_id_specific_bitwidth_not_match, (bitwidth & bitwidth_eight_a) ? 8 : 16, 8 * len); len = bitwidth & bitwidth_eight_a ? 1 : 2;} if (len==1) { write1(byte); write1(num); } \
																					 else { write1((unsigned int)byte); write2(num); } return true; }
#define as_xy(  op, byte) if (is(op)) { if(!explicitlen && !hexconstant) asar_throw_warning(0, warning_id_implicitly_sized_immediate); if (!check_immediate_bitwidth_xy(len)) { asar_throw_warning(0, warning_id_specific_bitwidth_not_match, (bitwidth & (bitwidth_eight_xy | bitwidth_sixteen_xy)), 8 * len); len = bitwidth & bitwidth_eight_xy ? 1 : 2; } if (len==1) { write1(byte); write1(num); } \
																					 else {  write1((unsigned int)byte); write2(num); } return true; }
